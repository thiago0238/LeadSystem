...
type SendCertificatesInput =
  | { type: "course"; courseId: number }
  | { type: "leads"; leadIds: number[] };
  ...
  try {
    ...
    if (data.type === "course") {
      ...
      leads = await prisma.lead.findMany({
        where: {
          OR: [
            { primaryCourseId: data.courseId },
            { secondaryCourseId: data.courseId },
          ],
        },
        include: {
          primaryCourse: true,
    ...
    if (leads.length === 0) {
      return {
        success: false,
        error: "Nenhum lead encontrado para envio de certificados",
      };
    }
      ...
      leads.map(async (lead) => {
        try {
          ...
          await sendEmail({
            ...
            attachments: [
              {
                filename: `certificado_${courseForCertificate.name.replace(
                  /\s+/g,
                  "_"
                )}.pdf`,
                content: pdfBuffer,
